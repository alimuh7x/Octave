...
for step = 1:nsteps
    ...
    Energy = trapz(x, f_local);
    EnergyN(end+1,1) = abs(Energy);

    % Visualization
    Delta_f = (f_beta - f_alpha) .* phi_alpha .* phi_beta;
    Delta_f_avg = mean(Delta_f) * 500;
    timeN(end+1,1) = step * dt;
    DrivingForceN(end+1,1) = abs(Delta_f_avg);
    ...
    phi_dot = L_phi * ( sigma * (phi_xx - dW_dphi) + (6 / w) .* phi_alpha .* phi_beta .* Delta_f_avg );
    ...
    if mod(step, 500) == 0
        ...
        myprint("Driving Force       : ", Delta_f_avg);
        fprintf('=================================================================\n');


        Subplot(i, phi_dot, Delta_f, 'x', 'phi_dot', 'Driving Force', '1PhaseDot_and_DrivingForce.png', true);
        ...
        SinglePlot(timeN, DrivingForceN, 'time', 'DrivingForce', 'DrivingForce.png', true);
        SinglePlot(timeN, EnergyN, 'time', 'Energy', 'Energy.png', true);

        dg_alpha = c - Ceq_alpha;
        dg_beta  = c - Ceq_beta;
        f_plot.add_and_plot(i, dg_alpha, dg_beta,  'x', 'dg_alpha', 'dg_beta');
        %P.add_plot(i, c, "x", "C");
